...
static std::unique_ptr<DatabaseCursor> GetTypeCursor(DatabaseBatch& batch, const std::string& type)
{
...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    cursor = GetTypeCursor(batch, DBKeys::WATCHS);
    if (!cursor) {
    ...
    // Load watchonly meta
    cursor = GetTypeCursor(batch, DBKeys::WATCHMETA);
    if (!cursor) {
    ...
    // Load keypool
    cursor = GetTypeCursor(batch, DBKeys::POOL);
    if (!cursor) {
...
static DBErrors LoadDescriptorWalletRecords(CWallet* pwallet, DatabaseBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    std::unique_ptr<DatabaseCursor> cursor = GetTypeCursor(batch, DBKeys::WALLETDESCRIPTOR);
    if (!cursor) {
    ...
    while (true) {
        ...
        std::unique_ptr<DatabaseCursor> sub_cursor = batch.GetPrefixCursor(prefix);
        if (!sub_cursor) {
        ...
        sub_cursor = batch.GetPrefixCursor(prefix);
        if (!sub_cursor) {
...
static DBErrors LoadAddressBookRecords(CWallet* pwallet, DatabaseBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    // Load name record
    std::unique_ptr<DatabaseCursor> cursor = GetTypeCursor(batch, DBKeys::NAME);
    if (!cursor) {
    ...
    // Load purpose record
    cursor = GetTypeCursor(batch, DBKeys::PURPOSE);
    if (!cursor) {
    ...
    cursor = GetTypeCursor(batch, DBKeys::DESTDATA);
    if (!cursor) {
...
static DBErrors LoadTxRecords(CWallet* pwallet, DatabaseBatch& batch, std::vector<uint256> upgraded_txs, bool& any_unordered) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    // Load tx record
    std::unique_ptr<DatabaseCursor> cursor = GetTypeCursor(batch, DBKeys::TX);
    if (!cursor) {
        pwallet->WalletLogPrintf("Error getting database cursor for address book names\n");
        return DBErrors::CORRUPT;
    }
    ...
    cursor = GetTypeCursor(batch, DBKeys::LOCKED_UTXO);
    if (!cursor) {
        pwallet->WalletLogPrintf("Error getting database cursor for address book names\n");
        return DBErrors::CORRUPT;
    }
...
static DBErrors LoadActiveSPKMs(CWallet* pwallet, DatabaseBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    for (auto& spk_key : {DBKeys::ACTIVEEXTERNALSPK, DBKeys::ACTIVEINTERNALSPK}) {
        std::unique_ptr<DatabaseCursor> cursor = GetTypeCursor(batch, spk_key);
        if (!cursor) {
