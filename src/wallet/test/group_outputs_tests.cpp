...
public:
                     ...
                     int expected_size)
    {
        std::vector<OutputGroup> groups = GroupOutputs(*wallet,
                                                       coins,
                                                       makeSelectionParams(avoid_partial_spends),
                                                       filter,
                                                       positive_only);
...
BOOST_AUTO_TEST_CASE(outputs_grouping_tests)
{
    ...
    const CScript& dest = GetScriptForDestination(wallet->GetNewDestination(OutputType::BECH32, "").GetObj());
    ...
    const CScript& dest2 = GetScriptForDestination(wallet->GetNewDestination(OutputType::P2SH_SEGWIT, "").GetObj());
    for (unsigned long i = 0; i < GROUP_SIZE; i++) {
    ...
    const CScript& dest3 = GetScriptForDestination(wallet->GetNewDestination(OutputType::BECH32, "").GetObj());
    ...
    const CScript& dest4 = GetScriptForDestination(wallet->GetNewDestination(OutputType::BECH32, "").GetObj());
    ...
    const CScript& dest5 = GetScriptForDestination(wallet->GetNewDestination(OutputType::BECH32, "").GetObj());
    ...
    const CScript& dest7 = GetScriptForDestination(wallet->GetNewDestination(OutputType::BECH32, "").GetObj());
