        ...
        bool sign)
{
    ...
    if (txr_ungrouped->fee > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        ...
        const auto& txr_grouped = res_tx_grouped.GetObj();
        const bool use_aps{txr_grouped ? (txr_grouped->fee <= txr_ungrouped->fee + wallet.m_max_aps_fee) : false};
        TRACE5(coin_selection, aps_create_tx_internal, wallet.GetName().c_str(), use_aps, txr_grouped != nullptr,
               txr_grouped ? txr_grouped->fee : 0, txr_grouped ? txr_grouped->change_pos : 0);
        if (txr_grouped) {
